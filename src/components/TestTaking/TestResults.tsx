...
const TestResults: React.FC = (): ReactElement => {
  ...
  const { 
    ...
    // Use mastery score data directly from the context (snapshot-based approach)
  // Initialize with context values or fallback to state
  const [masteryData, setMasteryData] = useState<MasteryData>({
    previousScore: masteryScoreBefore || null,
    newScore: masteryScoreAfter || null,
    change: masteryScoreChange || null
  });
  ...
  // When component mounts, update mastery score data if needed
  useEffect(() => {
    // In the new snapshot-based approach, mastery data comes directly from context
    // We only need to update our local state if context values change
    if (masteryScoreBefore !== null && masteryScoreAfter !== null && masteryScoreChange !== null) {
      console.log('Mastery data from test snapshot:', {
        before: masteryScoreBefore,
        after: masteryScoreAfter,
        change: masteryScoreChange
      });
      
      setMasteryData({
        previousScore: masteryScoreBefore,
        newScore: masteryScoreAfter,
        change: masteryScoreChange
      });
    }
    // If mastery data is not available in context but we have a test ID, 
    // it will be loaded by the loadHistoricalTest call in the other useEffect
  }, [masteryScoreBefore, masteryScoreAfter, masteryScoreChange, testId]);
