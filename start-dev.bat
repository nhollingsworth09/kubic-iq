...
REM Option to reset the database entirely (fresh start)
set /p choice="Do you want to completely RESET the database? (This will delete ALL data) (Y/N): "
if /i "%choice%"=="Y" (
    ...
    echo Installing server dependencies...
    cd /d %~dp0\server
    call npm install --no-audit --no-fund    echo Ensuring bcryptjs is properly installed...
    ...
    echo Skipping database reset. Will use existing database.
)
